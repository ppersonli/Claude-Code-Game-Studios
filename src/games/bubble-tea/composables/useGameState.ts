...
export interface BubbleTeaState {
...
export function createInitialState(): BubbleTeaState {
  return {
...
export function startStandardGame(state: BubbleTeaState): void {
  Object.assign(state, {
    score: 0, level: 1, combo: 0, maxCombo: 0, perfectCount: 0,
    drinksServed: 0, customersServed: 0, maxCustomers: 10,
    cupContents: [], currentOrder: [], currentCustomer: null,
...
export function startDailyGame(state: BubbleTeaState, modifier: DailyModifier): void {
  Object.assign(state, {
    score: 0, level: 1, combo: 0, maxCombo: 0, perfectCount: 0,
...
export function nextCustomer(state: BubbleTeaState): void {
...
export function addToCup(state: BubbleTeaState, ingredient: Ingredient): boolean {
  if (state.cupContents.length >= 6) return false
  state.cupContents.push(ingredient)
  return true
}

export function serveDrink(state: BubbleTeaState): ServeResult | null {
...
export function resetCup(state: BubbleTeaState): void {
  state.cupContents = []
}

export function endGame(state: BubbleTeaState): void {
  clearTimer(state)
...
export function unlockItem(state: BubbleTeaState, type: 'ingredient' | 'customer', id: string, cost: number): boolean {
...
export function checkAchievements(state: BubbleTeaState): string[] {
...
export function clearTimer(state: BubbleTeaState): void {
...
export function addFloatText(state: BubbleTeaState, text: string, color: string): void {
  const id = ++effectId
...
export function addScorePopup(state: BubbleTeaState, text: string, color: string): void {
  const id = ++effectId

...
export class MenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const bubble = this.add.circle(bx, by, br, 0xFFB7C5, 0.15)
      this.tweens.add({
        targets: bubble,
        y: -50,
    ...
    // Title
    this.add.text(GAME_W / 2, 180, 'MOCHI', {
      fontSize: '68px', fontFamily: 'Arial, sans-serif', color: '#FFB7C5', fontStyle: 'bold',
    }).setOrigin(0.5)

    this.add.text(GAME_W / 2, 260, 'MERGE', {
      fontSize: '68px', fontFamily: 'Arial, sans-serif', color: '#FF69B4', fontStyle: 'bold',
    }).setOrigin(0.5)

    this.add.text(GAME_W / 2, 320, '🍡 Mochi Merge Game', {
      fontSize: '18px', fontFamily: 'Arial, sans-serif', color: '#FF69B4',
    }).setOrigin(0.5)

    // Coins + stats bar
    this.add.text(GAME_W / 2, 360, `💰 ${save.coins}  |  🏆 Best: ${save.highScore}  |  🍡 ${save.stats.totalMerges} merges`, {
      fontSize: '14px', fontFamily: 'Arial, sans-serif', color: '#D81B60',
    }).setOrigin(0.5)

    // Play button
    const btnGfx = this.add.graphics()
    btnGfx.fillStyle(0xFFB7C5, 1)
    btnGfx.fillRoundedRect(GAME_W / 2 - 80, 400, 160, 55, 15)

    this.add.text(GAME_W / 2, 427, '▶ Play', {
      fontSize: '30px', fontFamily: 'Arial, sans-serif', color: '#FFFFFF', fontStyle: 'bold',
    }).setOrigin(0.5)

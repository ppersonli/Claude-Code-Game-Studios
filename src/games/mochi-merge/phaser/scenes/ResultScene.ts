...
export class ResultScene extends Phaser.Scene {
  ...
  create(data: { score?: number; highScore?: number; scoreCoins?: number; dailyCoins?: number; newAchievements?: AchievementInfo[] }): void {
    ...
    this.add.text(GAME_W / 2, 170, 'Game Over', {
      fontSize: '48px', fontFamily: 'Arial, sans-serif', color: '#FF69B4', fontStyle: 'bold',
    }).setOrigin(0.5)

    const scoreText = this.add.text(GAME_W / 2, 240, `Score: ${score}`, {
      fontSize: '32px', fontFamily: 'Arial, sans-serif', color: '#FFB7C5', fontStyle: 'bold',
    }).setOrigin(0.5)

    this.add.text(GAME_W / 2, 280, `Best: ${highScore}`, {
      fontSize: '20px', fontFamily: 'Arial, sans-serif', color: '#D81B60',
    }).setOrigin(0.5)
    ...
    this.add.text(GAME_W / 2, coinY, coinParts.join(' '), {
      fontSize: '18px', fontFamily: 'Arial, sans-serif', color: '#D81B60',
    }).setOrigin(0.5)
    coinY += 25
    ...
    if (achievements.length > 0) {
      for (const a of achievements.slice(0, 3)) {
        this.add.text(GAME_W / 2, coinY, `${a.emoji} ${a.name} +💰${a.reward}`, {
          fontSize: '14px', fontFamily: 'Arial, sans-serif', color: '#FF69B4',
        }).setOrigin(0.5)
        coinY += 20
      }
    }
    ...
    this.add.text(GAME_W / 2, playY + 80, '🏠 Menu', {
      fontSize: '20px', fontFamily: 'Arial, sans-serif', color: '#FF69B4',
